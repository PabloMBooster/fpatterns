...
projectmodel <- function(
  ...
  #   stop("Debe proporcionar 'species_data'.")
  # }
  
  # if (is.null(rownames(species_data)) ||
  #     !sp %in% rownames(species_data)) {
  #   stop("La especie seleccionada no se encuentra en 'species_data'.")
  # }
  
  # N debe ser una matriz: tallas x réplicas.
  if (is.null(dim(N))) {
    N <- matrix(N, ncol = 1)
  }
  ...
  #   stop("N debe contener las clases de talla como nombres de fila.")
  # }
  ...
  #   stop(
  #     "El número de clases de talla de N no coincide con ",
  #     "las clases definidas para la especie."
  #   )
  # }
  # 
  # if (length(catch) != nrow(N)) {
  #   stop("catch debe tener una observación por clase de talla.")
  # }
  
  matrixN <- array(
    data = NA_real_,
    dim = c(Ts + 1, nrow(N), ncol(N)),
    dimnames = list(
      paste0("time_", 0:Ts),
      rownames(N),
      colnames(N)
...
model <- function(
  ...
  for (t in seq_len(Ts)) {
    ...
    if (any(remaining_abundance < 0, na.rm = TRUE)) {
      warning(
        "Catch exceeds the available abundance in at least ",
...
lengthBasedModel <- function(
  ...
  progress <- progress::progress_bar$new(
    format = " Projecting bootstrap samples [:bar] :percent eta: :eta",
    total = n_boot,
    clear = FALSE,
    width = 60
  )
  ...
  for (bootstrap_id in seq_len(n_boot)) {
    ...
    abundance_current <- matrix(
      survey_data[, bootstrap_id] * survey_unit,
      ncol = 1,
  ...
  message(
    "Abundance unit: ", survey_unit, " individuals\n",
    "Biomass unit: tonnes\n",
